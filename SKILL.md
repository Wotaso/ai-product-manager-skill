...
- Real user notifications require a successful external channel such as native OpenClaw cron delivery, `deliveries.command`, Discord, Slack webhook, or generic webhook. OpenClaw cron delivery defaults to `automation.openclawCron.delivery.channel = "last"` so it uses the chat/social channel already connected to that OpenClaw instance, whether that is Discord, Slack, Telegram, WhatsApp, Matrix, Teams, or another supported channel. A local outbox write must never be presented as an externally delivered human alert.
- The runner is the only delivery owner. Do not add a second CLI/orchestrator send pass after the runtime finishes. Native OpenClaw/Hermes cron must inspect the newest proof, render its exact structured notification only when native delivery still owns it, and return `HEARTBEAT_OK` when `externalDeliverySent=true`, the category is disabled, or the run was suppressed/unchanged/not due. A state-file lock prevents OpenClaw and Hermes from running the same state concurrently; state writes are atomic. Failed growth-summary and runner-failure targets remain receipt-aware and retry only pending targets.
